...
$jscomp.polyfill("String.prototype.startsWith",function(b){return b?b:function(a,c){var b=$jscomp.checkStringArgs(this,a,"startsWith");a+="";var g=b.length,l=a.length;c=Math.max(0,Math.min(c|0,b.length));for(var q=0;q<l&&c<g;)if(b[c++]!=a[q++])return!1;return q>=l}},"es6","es3");$jscomp.findInternal=function(b,a,c){b instanceof String&&(b=String(b));for(var d=b.length,g=0;g<d;g++){var l=b[g];if(a.call(c,l,g,b))return{i:g,v:l}}return{i:-1,v:void 0}};
...
$jscomp.polyfill("Math.trunc",function(b){return b?b:function(a){a=Number(a);if(isNaN(a)||Infinity===a||-Infinity===a||0===a)return a;var c=Math.floor(Math.abs(a));return 0>a?-c:c}},"es6","es3");
var bgss=function(){function b(){var a=new Date;return a.getFullYear()+"/"+("0"+(a.getMonth()+1)).substr(-2)+"/"+("0"+a.getDate()).substr(-2)+"-"+("0"+a.getHours()).substr(-2)+":"+("0"+a.getMinutes()).substr(-2)+":"+("0"+a.getSeconds()).substr(-2)}function a(a){t.score(k[0],k[a]);var c=k[a].score;e.debug&&console.log("player "+a+" score:",c,k[a]);$("#score-"+a).is("input")?$("#score-"+a).val(c):$("#score-"+a).text(c);$("#result-"+a).text(c)}function c(c){var h=0;t.common&&(h+=t.common(k[0]));if(h||
0===c)for(e.debug&&console.log("general:",k[0]),c=1;c<=e.players;c++)a(c);else a(c);h={};var d=0;for(c=1;c<=e.players;c++)k[c].score in h?d++:h[k[c].score]=1;d?$("#tie").show():$("#tie").hide();$("#date").text(b())}function d(a){var c=0;if("checkbox"==a.attr("type"))return a.prop("checked");if(a.hasClass("togglable"))return!a.hasClass("hollow");var b=a.val();if(!b)return 0;var h=b.startsWith(".")?-parseInt(b.substr(1)):parseInt(b);h.toString()!=b&&c++;b=a.attr("min");"undefined"!==typeof b&&h<b&&
(h=b,c++);b=a.attr("max");"undefined"!==typeof b&&h>b&&(h=b,c++);c&&a.val(h);return h}function g(a){var c=[];$.each(u,function(b,h){h[a]&&c.push([b,h[a]])});c.sort(function(a,c){return a[1]>c[1]?1:a[1]<c[1]?-1:0});$("#game").data(a,c)}function l(a,c){a=a.clone().attr("id","player"+c);var b;a.find("*").each(function(a){(b=$(this).attr("id"))&&$(this).attr("id",b+"-"+c);(b=$(this).attr("for"))&&$(this).attr("for",b+"-"+c)});var h=1;a.find("input").each(function(a){$(this).attr("data-index",c+("0"+h).substr(-2));
a=$(this).attr("min");var b=$(this).attr("max");"undefined"!==typeof a&&"undefined"!==typeof b&&$(this).attr("placeholder",a+"-"+b);"undefined"===typeof a||"0"===a||$(this).attr("value")||$(this).attr("value",a);h++});return a}function q(){$("select[data-init]").each(function(a){a=$(this).data("init").split("/").map(function(a){return parseInt(a)});for(var c=a[0];c<=a[2];c++){var b=$("<option>").attr("value",c).text(c);c==a[1]&&b.attr("selected","selected");$(this).append(b)}})}function n(){var a;
for(a=0;a<=e.players;a++)k.push({id:a});k[0].sum={};var b=$("#tabs").children("li").last();for(a=e.players;1<=a;a--){var f=$("<a>").attr("href","#player"+a).text(a);f=$("<li>").addClass("tabs-title").append(f);b.before(f);b=f}b=$("#_player_");for(a=1;a<=e.players;a++)$("#tabs-content").append(l(b,a));b.remove();$("#tabs").on("change.zf.tabs",function(){var a=$("#tabs-content").find(".tabs-panel.is-active").attr("id"),c=u[e.game][e.language];if(a&&a.startsWith("player")){a=parseInt(a.substr(6));var b=
'<span id="score-'+a+'">'+k[a].score+"</span>";$("#title").html(c+" - "+e.player[a-1]+" = "+b)}else $("#title").html(c)});$("#tabs-content").find("input,select").each(function(a){var b=$(this).attr("id");if(b&&(a=$(this).closest("div.tabs-panel").attr("id"))){if(a.startsWith("player")){var h=parseInt(a.substr(-1));var f=b.substr(0,b.length-2)}else h=0,f=b;b in r&&("checkbox"==$(this).attr("type")?r[b]?$(this).prop("checked",!0):$(this).prop("checked",!1):$(this).val(r[b]));k[h][f]=d($(this));$(this).change(function(){k[h][f]=
d($(this));if($(this).hasClass("highlander")){var a=f,y=h;if(k[y][a])for(var n=1;n<=e.players;n++)n!=y&&k[n][a]&&$("#"+a+"-"+n).click()}r[b]=k[h][f];z();c(h)})}});$("#tabs-content").find("a.togglable").each(function(a){var b=$(this).attr("id");if(b&&(a=$(this).closest("div.tabs-panel").attr("id"))){if(a.startsWith("player")){var h=parseInt(a.substr(-1));var e=b.substr(0,b.length-2)}else h=0,e=b;b in r&&(r[b]?$(this).removeClass("hollow"):$(this).addClass("hollow"));k[h][e]=d($(this));$(this).click(function(){$(this).hasClass("hollow")?
($(this).removeClass("hollow"),k[h][e]=!0):($(this).addClass("hollow"),k[h][e]=!1);r[b]=k[h][e];z();c(h)})}});for(a=1;a<=e.players;a++)c(a);Foundation.reInit($("#tabs"));$(".accordion").foundation()}function m(){$("body").on("keydown","input",function(a){13==a.which&&(a.preventDefault(),a=$(a.target).attr("data-index"))&&(a=parseInt(a)+1,$("[data-index="+a+"]").focus())})}function f(){$("#language").change(function(){e.language=$(this).val();e.debug&&console.log("language changed:",e.language);w();
v()});$("#game").change(function(){e.game=$(this).val();e.debug&&console.log("game changed:",e.game);p();v()});$("#players").change(function(){e.players=$(this).val();e.debug&&console.log("players changed:",e.players);x();v()});$("body").find("input").each(function(a){if((a=$(this).attr("id"))&&a.startsWith("player")){var c=parseInt(a.substr(6));$(this).change(function(){e.player[c-1]=$(this).val().trim();e.player[c-1]!=$(this).val()&&$(this).val(e.player[c-1]);e.debug&&console.log("player "+c+" changed:",
e.player[c-1]);v()})}});$("#clear").click(function(){for(var a=1;7>=a;a++)e.player[a-1]="",$("#player"+a).val("");v()});$("#next").click(function(){var a=window.location,c=e.game+".html#reset=1&language="+e.language+"&players="+e.players;e.debug&&(c+="&debug=1");for(var b=[],d=1;d<=e.players;d++)b.push(e.player[d-1]||d);c+="&names="+b.map(encodeURI).join(",");a.href=c})}function w(){var a="[lang="+e.language+"]";$("body").find("[lang]").not(a).hide();$("body").find(a).show();$("body").find("select.langaware").each(function(a){var c=
$(this);a=c.val();var b=c.data(e.language);c.empty();$.each(b,function(a,b){c.append($("<option>").attr("value",b[0]).html(b[1]))});a&&c.val(a)})}function p(){var a=u[e.game];$("#players").children("option").each(function(c){c=$(this).attr("value");a.min<=c&&c<=a.max?$(this).show():$(this).hide()});var c=0;e.players<a.min?c=a.min:e.players>a.max&&(c=a.max);c&&(e.players=c,$("#players").val(e.players),x());a.min==a.max?$("#players").prop("disabled","disabled"):$("#players").prop("disabled",!1)}function x(){$("body").find("div").each(function(a){(a=
$(this).attr("id"))&&a.startsWith("div")&&(parseInt(a.substr(3))<=e.players?$(this).show():$(this).hide())})}function A(){$("#language").val(e.language);w();$("#game").val(e.game);p();$("#players").val(e.players);x();$.each(e.player,function(a,c){$("#player"+(a+1)).val(c)})}function z(){"undefined"!==typeof Storage&&(e.debug&&console.log("store",r),localStorage.setItem("bgss.game",JSON.stringify(r)))}function v(){if("undefined"!==typeof Storage){e.debug&&console.log("store",e);var a={};$.each(e.player,
function(c,b){a[b]=1});e.names&&$.each(e.names,function(c,b){a[b]=1});delete a[""];e.names=Object.keys(a);e.names.sort();localStorage.setItem("bgss.main",JSON.stringify(e))}}function B(){var a=e.debug;if("undefined"!==typeof Storage){var c=localStorage.getItem("bgss.main");if(c){e=JSON.parse(c);"undefined"!==typeof a&&(e.debug=a);e.debug&&console.log("fetch",e);e.names&&$.each(e.names,function(a,c){$("datalist#names").append($("<option>").text(c))});return}}e={language:"en",game:"00164928",players:4,
...
"");if(a=window.location.pathname.match(/(\d+)\.html$/))if(t=u[a[1]])e.game=a[1],$("#title").html(t[e.language]),document.title="BGSS - "+$("#title").text();else throw"unexpected game: "+a[1];else throw"unexpected pathname: "+window.location.pathname;e.debug&&console.log("url2state",e)}var r={},k=[],u={},e={},t;return{define:function(a,c){u[a]=c},init_main:function(){var a=window.location.hash;a.length&&(a=a.match(/debug=(\d)/))&&(e.debug=parseInt(a[1]));g("en");g("fr");m();f();B();a=window.location.hash;
a.length&&(a=a.match(/^#(\d{8})$/))&&(e.game=a[1]);A()},init_game:function(){C();if("undefined"!==typeof Storage)if(e.reset)localStorage.removeItem("bgss.game");else{var a=localStorage.getItem("bgss.game");a&&(r=JSON.parse(a),e.debug&&console.log("fetch",r))}t.init&&t.init(e);a=$("#results table tbody");for(var c=1;c<=e.players;c++){var b=$("<tr>");b.append($("<td>").html(e.player[c-1]));b.append($("<td>").attr("id","result-"+c).text("0"));a.append(b)}q();n();m();t.setup&&t.setup(e);w();$("#tabs").children("li").first().click()},
update_majority:function(a,c){for(var b={},d=1;d<=e.players;d++)b[k[d][a]]||(b[k[d][a]]=[]),b[k[d][a]].push(d);d=Object.keys(b);d.sort(function(a,c){return parseInt(a)>parseInt(c)?-1:parseInt(a)<parseInt(c)?1:0});var f=0,n;$.each(d,function(d,e){d=b[e];for(n=e=0;n<d.length;n++)e+=c[f++];e=Math.floor(e/d.length);for(n=0;n<d.length;n++)k[d[n]][a+"_bonus"]=e})},update_sum:function(a,c){c=c||0;for(var b=1;b<=e.players;b++)c+=k[b][a];if(a in k[0].sum&&k[0].sum[a]==c)return 0;k[0].sum[a]=c;return 1}}}();bgss.define("00215471",function(){var b=7,a=[-3,1,3,4,7,8,10,11];return{en:"Wind the Film!",fr:"Photograph",min:2,max:4,init:function(a){b=2==a.players?5:3==a.players?6:7},setup:function(a){7>b&&$(".color7").hide();6>b&&$(".color6").hide()},score:function(c,d){d.score=0;for(c=1;c<=b;c++)if(c==d.regret)$("#player1 .color"+c).hide();else{$("#player1 .color"+c).show();d["c"+c+"-g"]&&(d.score+=5);var g=d.score,l=d["c"+c+"-s"];l>=a.length&&(l=a.length-1);d.score=g+a[l];d.score-=2*d["c"+c+"-b"]}}}}());bgss.define("00068448",function(){function b(a,b,g){return a*a+b*b+g*g+7*Math.min(a,b,g)}function a(a){if(0===a.wild)return b(a.tablets,a.compasses,a.gears);if(1===a.wild)return Math.max(b(a.tablets+1,a.compasses,a.gears),b(a.tablets,a.compasses+1,a.gears),b(a.tablets,a.compasses,a.gears+1));if(2===a.wild)return Math.max(b(a.tablets+2,a.compasses,a.gears),b(a.tablets,a.compasses+2,a.gears),b(a.tablets,a.compasses,a.gears+2),b(a.tablets+1,a.compasses+1,a.gears),b(a.tablets+1,a.compasses,a.gears+1),
...
(a.score+=3)}}}());bgss.define("00031260",function(){function b(a,b){a>=b.length&&(a=b.length-1);return b[a]}var a=[-1,-1,1,2,3,4],c=[-1,1,2,3,4],d=[-1,1,1,1,2,2,3,3,4],g=[-1,1,2,3,4],l=[-1,1,1,1,2,2,3,3,4],q=[-1,1,1,2,2,3,3,4],n=[-1,1,2,2,3,3,4];return{en:"Agricola",fr:"Agricola",min:1,max:5,score:function(m,f){f.score=0;f.score+=b(f.plowed,a);f.score+=b(f.pastures,c);f.score+=b(f.grain,d);f.score+=b(f.vegetables,g);f.score+=b(f.sheep,l);f.score+=b(f.boar,q);f.score+=b(f.cattle,n);f.score-=f.unused;f.score+=f.stables;
f.score+=f.clay;f.score+=2*f.stone;f.score+=3*f.family;f.score+=f.card;f.score+=f.bonus;f.score-=3*f.begging}}}());bgss.define("00227224",function(){function b(){var a,b=[],c=0,d;for(a=0;a<q;a++){c+=2*g[a];c+=l[a];var p=g[a]+l[a];0!=p&&(b[p]?b[p].push(a):b[p]=[a])}$("#value").text(c);for(a=1;a<=q;a++)$("#score-"+a).val(0);for(p=10;0<p;p--)if(b[p]){for(a=d=0;a<b[p].length;a++)d+=c,c=2==q?Math.trunc(c/3):Math.trunc(c/2);1<b[p].length&&(d=2==q?0:Math.trunc(d/b[p].length));for(a=0;a<b[p].length;a++)$("#score-"+(b[p][a]+1)).val(d);if(0==c)break}}function a(){var a;for(a=0;a<q;a++)g[a]=0,$("#sections-"+(a+1)).val(0),
l[a]=0,$("#ornaments-"+(a+1)).val(0);b()}function c(a){a.data.array[a.data.index]=parseInt(a.target.value,10);b()}function d(a,b){var c;a=a.clone().attr("id","row"+b);a.find("*").each(function(a){(c=$(this).attr("id"))&&$(this).attr("id",c+"-"+b)});return a}var g=[],l=[],q=0;return{en:"The Red Cathedral",fr:"The Red Cathedral",min:2,max:4,score:function(){},setup:function(b){var m;q=b.players;var f=$("#row");for(m=1;m<=b.players;m++)$("#rows").append(d(f,m)),g.push(0),l.push(0);f.remove();$("#reset").on("click",
