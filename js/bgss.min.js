...
f.score+=f.clay;f.score+=2*f.stone;f.score+=3*f.family;f.score+=f.card;f.score+=f.bonus;f.score-=3*f.begging}}}());bgss.define("00312484",function(){return{en:"Lost Ruins of Arnak",fr:"Les Ruines Perdues de Narak",min:1,max:4,score:function(b,a){a.score=0;a.score+=a.research;a.score+=a.temple;a.score+=3*a.idol;a.score+=a.empty;a.score+=5*a.guardian;a.score+=a.item;a.score+=a.artifact;a.score-=a.fear1;a.score-=2*a.fear2}}}());bgss.define("00227224",function(){function b(){var a,b=[],c=0,d;for(a=0;a<p;a++){c+=2*g[a];c+=l[a];var n=g[a]+l[a];0!=n&&(b[n]?b[n].push(a):b[n]=[a])}$("#value").text(c);for(a=1;a<=p;a++)$("#score-"+a).val(0);for(n=10;0<n;n--)if(b[n]){for(a=d=0;a<b[n].length;a++)d+=c,c=2==p?Math.floor(c/3):Math.floor(c/2);1<b[n].length&&(d=2==p?0:Math.floor(d/b[n].length));for(a=0;a<b[n].length;a++)$("#score-"+(b[n][a]+1)).val(d);if(0==c)break}}function a(){var a;for(a=0;a<p;a++)g[a]=0,$("#sections-"+(a+1)).val(0),
