...
var bgss=function(){function b(){var a=new Date;return a.getFullYear()+"/"+("0"+(a.getMonth()+1)).substr(-2)+"/"+("0"+a.getDate()).substr(-2)+"-"+("0"+a.getHours()).substr(-2)+":"+("0"+a.getMinutes()).substr(-2)+":"+("0"+a.getSeconds()).substr(-2)}function a(a){n.score(k[0],k[a]);var c=k[a].score;e.debug&&console.log("player "+a+" score:",c,k[a]);$("#score-"+a).is("input")?$("#score-"+a).val(c):$("#score-"+a).text(c);$("#result-"+a).text(c)}function c(c){var h=0;n.common&&(h+=n.common(k[0]));if(h||
...
(h=b,c++);b=a.attr("max");"undefined"!==typeof b&&h>b&&(h=b,c++);c&&a.val(h);return h}function g(a){var c=[];$.each(q,function(b,h){h[a]&&c.push([b,h[a]])});c.sort(function(a,c){return a[1]>c[1]?1:a[1]<c[1]?-1:0});$("#game").data(a,c)}function l(a,c){a=a.clone().attr("id","player"+c);var b;a.find("*").each(function(a){(b=$(this).attr("id"))&&$(this).attr("id",b+"-"+c);(b=$(this).attr("for"))&&$(this).attr("for",b+"-"+c)});var h=1;a.find("input").each(function(a){$(this).attr("data-index",c+("0"+h).substr(-2));
a=$(this).attr("min");var b=$(this).attr("max");"undefined"!==typeof a&&"undefined"!==typeof b&&$(this).attr("placeholder",a+"-"+b);"undefined"===typeof a||"0"===a||$(this).attr("value")||$(this).attr("value",a);h++});return a}function p(){$("select[data-init]").each(function(a){a=$(this).data("init").split("/").map(function(a){return parseInt(a)});for(var c=a[0];c<=a[2];c++){var b=$("<option>").attr("value",c).text(c);c==a[1]&&b.attr("selected","selected");$(this).append(b)}})}function t(){var a;
...
'<span id="score-'+a+'">'+k[a].score+"</span>";$("#title").html(c+" - "+e.player[a-1]+" = "+b)}else $("#title").html(c)});$("#tabs-content").find("input,select").each(function(a){var b=$(this).attr("id");if(b&&(a=$(this).closest("div.tabs-panel").attr("id"))){if(a.startsWith("player")){var h=parseInt(a.substr(-1));var f=b.substr(0,b.length-2)}else h=0,f=b;b in m&&("checkbox"==$(this).attr("type")?m[b]?$(this).prop("checked",!0):$(this).prop("checked",!1):$(this).val(m[b]));k[h][f]=d($(this));$(this).change(function(){k[h][f]=
d($(this));if($(this).hasClass("highlander")){var a=f,x=h;if(k[x][a])for(var g=1;g<=e.players;g++)g!=x&&k[g][a]&&$("#"+a+"-"+g).click()}m[b]=k[h][f];y();c(h)})}});$("#tabs-content").find("a.togglable").each(function(a){var b=$(this).attr("id");if(b&&(a=$(this).closest("div.tabs-panel").attr("id"))){if(a.startsWith("player")){var h=parseInt(a.substr(-1));var e=b.substr(0,b.length-2)}else h=0,e=b;b in m&&(m[b]?$(this).removeClass("hollow"):$(this).addClass("hollow"));k[h][e]=d($(this));$(this).click(function(){$(this).hasClass("hollow")?
($(this).removeClass("hollow"),k[h][e]=!0):($(this).addClass("hollow"),k[h][e]=!1);m[b]=k[h][e];y();c(h)})}});for(a=1;a<=e.players;a++)c(a);Foundation.reInit($("#tabs"));$(".accordion").foundation()}function u(){$("body").on("keydown","input",function(a){13==a.which&&(a.preventDefault(),a=$(a.target).attr("data-index"))&&(a=parseInt(a)+1,$("[data-index="+a+"]").focus())})}function f(){$("#language").change(function(){e.language=$(this).val();e.debug&&console.log("language changed:",e.language);v();
