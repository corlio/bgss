...
f.score+=f.clay;f.score+=2*f.stone;f.score+=3*f.family;f.score+=f.card;f.score+=f.bonus;f.score-=3*f.begging}}}());bgss.define("00258389",function(){return{en:"Futuropia",fr:"Futuropia",min:1,max:4,score:function(b,a){a.score=0;a.score+=5*a.people;a.score+=a.quarters;a.score+=Math.floor((a.energy+a.food+a.money)/10);b.variant1?(a.score-=a.loans,a.score+=a.actions):(a.score-=3*a.loans,a.score+=3*a.actions);b.variant2?(a.score+=2*a.xenergy,a.score+=2*a.xfood):(a.score+=a.xenergy,a.score+=a.xfood)}}}());bgss.define("00228959",function(){return{en:"Alubari",fr:"Alubari",min:1,max:5,setup:function(b){1==b.players&&$("#equipment-1").parent().parent().hide()},score:function(b,a){a.score=0;a.score+=a.estates;a.score+=a.stations;a.score+=a.tracks;a.score+=a.contracts;a.score+=a.equipment;a.score+=a.chai;a.score+=a.tea/2}}}());bgss.define("00216132",function(){var b,a;return{en:"Clans of Caledonia",fr:"Clans of Caledonia",min:1,max:4,init:function(c){3<=c.players?(b=[12,6,0,0],a=[18,12,6,0]):(b=[8,0],a=[12,0])},common:function(c){var d=0+bgss.update_sum("cotton",.1);d+=bgss.update_sum("tobacco",.2);if(d+=bgss.update_sum("sugar",.3)){var g=["cotton","tobacco","sugar"];g.sort(function(a,b){return c.sum[a]>c.sum[b]?1:c.sum[a]<c.sum[b]?-1:0});c[g[0]]=5;c[g[1]]=4;c[g[2]]=3;$("#vp_cotton").text(c.cotton);$("#vp_tobacco").text(c.tobacco);
